...
<?php
    ...
    $gal_item = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."photocrati_galleries WHERE id = " . ((int)$_GET['prod_id']), ARRAY_A);
	...
	jQuery("#addto").click(function()
	{
		...
		jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('ecomm-add-to-cart.php'); ?>", data: 'action=add&id=<?php echo $_GET['prod_id']; ?>', success: function(data)
		{
			...
			jQuery('[id^=qty_]').each(function()
			{
				...
				var currentId = jQuery(this).attr('id');
				jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('ecomm-add-to-cart-quantities.php'); ?>", data: 'action=add&id=<?php echo $_GET['prod_id']; ?>&size_id='+currentId.substr(4)+'&qty='+jQuery(this).val()+'&total='+jQuery("#line_"+currentId.substr(4)).val(), success: function(data)
				{
...
<body style="background:#ffffff;">
	...
	<div id="cart_wrapper">
		...
		<div class="cart_data">
			...
			<?php
			$l = 1;
			$ecomm_ops = explode(',',$ecomm_options);
			while($l < 21) {
			if(in_array($l,$ecomm_ops)) { 
			?>
