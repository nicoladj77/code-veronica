...
jQuery(document).ready(function()
{
	...
	if(jQuery('input[name=gal_type]:checked').val() == '6' || jQuery('input[name=gal_type]:checked').val() == '7') {
		...
		jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/album-list-galleries.php'); ?>", data: 'edit=true&post=<?php echo $_GET['post']; ?>&gallery_id=<?php echo $_GET['gallery_id']; ?>', success: function(data)
		{
...
jQuery(document).ready(function()
{
        ...
        if (answer){
            ...
            jQuery('[id^=delete_img_]:checked').each(function(index) {
				...
				setTimeout(function(){
					...
					jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/delete-gallery-images.php'); ?>", data: "gallery_id=<?php echo $_GET['post']; ?>&img_id="+currentId.substr(11), success: function(data)
					{
    ...
    jQuery('#order_fname').click(function() {
    	fnGalleryOrder('filename');
    });
    
    jQuery('#order_title').click(function() {
    	fnGalleryOrder('title');
    });
    
    jQuery('#order_invert').click(function() {
			var sortList = jQuery('[id^=sortable_]').get();
			var sortParent = jQuery(sortList[0]).parent();
    ...
    jQuery('#import_nexgen').click(function(){
		...
		jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/get-nextgen-image.php'); ?>", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
		{
    ...
    jQuery('#import_nexgen_images').live('click', function(){
        ...
        if (answer){
            ...
            jQuery('[id^=nggallery_image_]').each(function(index) {
                ...
                jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/import-nextgen-images.php'); ?>", data: 'image='+fileName.replace("&","%26")+'&path='+jQuery('#nggallery_path_'+currentId.substr(16)).val()+'&gallery_id=<?php echo $_GET['post']; ?>', success: function(data)
				{  
    ...
    jQuery('#import_photocrati').click(function(){
		...
		jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/get-photocrati-image.php'); ?>", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
		{
    ...
    jQuery('#import_photocrati_images').live('click', function(){
        ...
        if (answer){
            ...
            jQuery('[id^=photocrati_image_]').each(function(index) {
                ...
                var fileTitle = jQuery(this).attr('title');
                var fileDesc = jQuery(this).attr('alt');
                jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/import-photocrati-images.php'); ?>", data: 'image='+fileName.replace("&","%26")+'&path='+jQuery('#photocrati_path_'+currentId.substr(17)).val()+'&gallery_id=<?php echo $_GET['post']; ?>', success: function(data)
				{  
		...
		function saveGallery() {
			...
			jQuery('[id^=sortable_]').each(function(index) {
				...
				jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/save-gallery-images-update.php'); ?>", data: 'gallery_id=<?php echo $_GET['gallery_id']; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&image_name='+jQuery('#gallery_image_'+currentId.substr(9)).val().replace("&","%26")+'&image_order='+ imgOrder.toString() +'&image_alt='+jQuery('#image_alt_'+currentId.substr(9)).val().replace("&","%26")+'&image_desc='+jQuery('#image_desc_'+currentId.substr(9)).val().replace("&","%26")+'&ecomm_options='+options, success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/update-gallery.php'); ?>", data: 'gallery_id=<?php echo $_GET['gallery_id']; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26")+'&gal_desc='+jQuery("#gal_desc").val().replace("&","%26")+'&gal_height='+jQuery("#gal_height").val()+'&gal_aspect_ratio='+jQuery("#gal_aspect_ratio").val(), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
							...
							jQuery.ajax({type: "POST", async: ajax_async, url: "<?php echo photocrati_gallery_file_uri('admin/get_galleries.php'); ?>", data: 'post_id=<?php echo $_GET['post']; ?>', success: function(data)
								{
...
<form name="create_gallery" id="create_gallery" method="post">
	...
	?>
        ...
        <div id="galleria_<?php echo $gallery->id; ?>">
            ...
            <p style="margin-right:120px;"><label>Title / Alt:</label><BR><input class="image-input-field" type="text" id="image_alt_<?php echo $nums; ?>" value="<?php if($gallery->image_alt) { echo htmlspecialchars_decode(stripslashes($gallery->image_alt)); } else { echo htmlspecialchars_decode($gallery->image_name); } ?>"></p>
