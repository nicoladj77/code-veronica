...
jQuery(document).ready(function()
{
	jQuery('input[name=gal_type]').change(function(){
		...
		if(jQuery('input[name=gal_type]:checked').val() == '6' || jQuery('input[name=gal_type]:checked').val() == '7') {
			...
			jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/album-list-galleries.php'); ?>", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
			{
		...
		function saveGallery() {
			...
			jQuery('[id^=gallery_image_]').each(function(index) {
				...
				jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/save-gallery-images.php'); ?>", data: 'gallery_id=<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&image_name='+jQuery(this).val().replace("&","%26")+'&image_order=0&image_alt='+jQuery('#image_alt_'+currentId.substr(14)).val().replace("&","%26")+'&image_desc='+jQuery('#image_desc_'+currentId.substr(14)).val().replace("&","%26"), success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/save-gallery.php'); ?>", data: 'gallery_id=<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?>&post_id=<?php echo $_GET['post']; ?>&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26")+'&gal_desc='+jQuery("#gal_desc").val().replace("&","%26")+'&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&gal_height='+jQuery("#gal_height").val()+'&gal_aspect_ratio='+jQuery("#gal_aspect_ratio").val(), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
						
							var img_html = '<img id="phgallery-<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php echo photocrati_gallery_file_uri('image/gallery-placeholder-' . $galnextid . '.gif'); ?>" alt="photocrati gallery" />';
	...
	jQuery('#delete_button').click(function(){
		...
		if (answer){
			...
			jQuery.ajax({type: "POST", url: "<?php echo photocrati_gallery_file_uri('admin/delete-gallery.php'); ?>", data: 'post_id=<?php echo $_GET['post']; ?>', success: function(data)
				{
