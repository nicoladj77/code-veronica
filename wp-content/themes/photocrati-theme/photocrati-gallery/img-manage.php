...
function add_media_button() {
	...
	$photocrati_upload_iframe_src = apply_filters('audio_upload_iframe_src', photocrati_gallery_file_uri('admin/upload.php'));
    ...
    if(!$_GET["post"]) {
        $photocrati_button .= '<a href="#" onclick="alert(\'You need to save your post or page before you can insert a Photocrati gallery. You can do this by inserting a title and clicking the Save Draft button.\')" id="add_gallery" title="' . $photocrati_title . '" onclick="return false;"><img src="' . photocrati_gallery_file_uri('image/new_gallery.gif') . '" alt="' . $photocrati_title . '" /></a>';
    } else {
        $photocrati_button .= '<a id="ph_gallery_button" style="clear:none;';
        if($u != 0) {
        //$photocrati_button .= 'display:none;';
        }
        $photocrati_button .= '" href="' . $photocrati_upload_iframe_src . '?post='.$post_ID.'&TB_iframe=true" id="add_gallery" class="thickbox" title="' . $photocrati_title . '" onclick="return false;"><img src="' . photocrati_gallery_file_uri('image/new_gallery.gif') . '" alt="' . $photocrati_title . '" /></a>';
    }
...
function insertGallery($atts) 
{
	...
	for ($i = 0; $i < $count; $i++)
	{
		...
		{
			$insertgallery .= ' title="' . $instance_image->Description . '"';
		}
		
		$insertgallery .= ' id="img_' . $gallery_id . '_' . ($i + 1) . '" rel="gallery_' . $gallery_id . '">';
		$insertgallery .= '<img src="' . $instance_image->ThumbURI . '" alt="' . $instance_image->NameHTML . '" />';
		$insertgallery .= '</a>';
	}
...
//-->
	...
	jQuery(document).ready(function() {
		...
		jQuery.ajax({type: "POST", url: "' . photocrati_gallery_file_uri('ecomm-cart-widget.php') . '", data: \'\', success: function(data)
        {
