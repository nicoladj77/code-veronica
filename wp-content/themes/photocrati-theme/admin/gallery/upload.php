...
$SQL = "SELECT * FROM ".$wpdb->prefix."photocrati_gallery_ids WHERE post_id = " . ((int)$_GET['post']) . "";	
...
jQuery(document).ready(function()
{
	jQuery('input[name=gal_type]').change(function(){
		...
		if(jQuery('input[name=gal_type]:checked').val() == '6' || jQuery('input[name=gal_type]:checked').val() == '7') {
			...
			jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/album-list-galleries.php", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
			{
	...
	jQuery('[id^=save_button_]').live('click', function(){
		...
		function saveGallery() {
			...
			jQuery('[id^=gallery_image_]').each(function(index) {
                var currentId = jQuery(this).attr('id');
				jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-gallery-images.php", data: 'gallery_id=<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&image_name='+jQuery(this).val().replace("&","%26")+'&image_order=0&image_alt='+jQuery('#image_alt_'+currentId.substr(14)).val().replace("&","%26")+'&image_desc='+jQuery('#image_desc_'+currentId.substr(14)).val().replace("&","%26"), success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-gallery.php", data: 'gallery_id=<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?>&post_id=<?php echo $_GET['post']; ?>&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26")+'&gal_desc='+jQuery("#gal_desc").val().replace("&","%26")+'&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&gal_height='+jQuery("#gal_height").val(), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
						
							var img_html = '<img id="phgallery-<?php echo $_GET['post']; ?>_<?php echo $galnextid; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php bloginfo('template_url'); ?>/admin/images/gallery-placeholder-<?php echo $galnextid; ?>.gif" alt="photocrati gallery" />';
							...
							jQuery.ajax({type: "POST", async: ajax_async, url: "<?php echo get_bloginfo('template_url'); ?>/admin/gallery/get_galleries.php", data: 'post_id=<?php echo $_GET['post']; ?>', success: function(data)
								{
									...
									parent.jQuery('#reinsert_button_<?php echo $_GET['post'].'_'.$galnextid; ?>').hide();
								}
							});
	...
	jQuery('#delete_button').click(function(){
		...
		if (answer){
			...
			jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/delete-gallery.php", data: 'post_id=<?php echo $_GET['post']; ?>', success: function(data)
				{
...
<form name="create_gallery" id="create_gallery" method="post" onSubmit="">
    ...
    <div id="gallery_types">
    	...
    	<div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="1" checked="checked" /> Slideshow<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_animated.jpg" id="gal_img_1" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="2" /> Blog Style<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_sstack.jpg" id="gal_img_2" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="3" /> Filmstrip<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_hfilm.jpg" id="gal_img_3" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="4" /> Thumbnails<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_thumbs.jpg" id="gal_img_4" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="5" /> E-Commerce<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_ecomm.jpg" id="gal_img_5" /></div>
    ...
    <div id="gallery_types">
		...
		<div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="6" /> Album - List<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/album_list.jpg" id="gal_img_6" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type" value="7" /> Album - Grid<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/album_grid.jpg" id="gal_img_7" /></div>
    ...
    <div style="float:left;width:18%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/gallery_nexgen.jpg" id="import_nexgen" value="Import from NextGen" onclick="return false;" />
    </div>
    <div style="float:left;width:18%;">
