...
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/jquery.ui.core.js"></script>
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/jquery.ui.widget.js"></script>
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/jquery.ui.mouse.js"></script>
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/jquery.ui.draggable.js"></script>
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/jquery.ui.sortable.js"></script>
<script type="text/javascript" src="<?php echo get_bloginfo('template_url'); ?>/admin/js/swfobject.js"></script>
...
jQuery(document).ready(function()
{
	...
	if(jQuery('input[name=gal_type]:checked').val() == '6' || jQuery('input[name=gal_type]:checked').val() == '7') {
		...
		jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/album-list-galleries.php", data: 'edit=true&post=<?php echo $_GET['post']; ?>&gallery_id=<?php echo $_GET['gallery_id']; ?>', success: function(data)
		{
...
jQuery(document).ready(function()
{
	...
	'buttonImg'	: '<?php bloginfo('template_url'); ?>/admin/images/upload_gallery.jpg',
	'cancelImg' : '<?php bloginfo('template_url'); ?>/admin/images/cancel_gallery.gif',
	'folder'    : '/galleries/post-<?php echo $_GET['post']; ?>/',
	'auto'      : true,
	'multi'     : true,
	'queueSizeLimit' : 99,
	'simUploadLimit' : 1,
	...
	'onComplete': function(event, queueID, fileObj, response, data) {
        ...
        if (browserName == 'Microsoft Internet Explorer') {
			<?php if(!function_exists('gd_info')) { ?>
				jQuery("#fileName")
				.fadeIn('slow')
				.append('<div id="sortable_'+nums+'"><div id="gallery_uploaded"><img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/'+eiptc[0].replace("%","%25")+'" align="absmiddle" style="height:90px;width:130px;"></div></div>');
			<?php } else { ?>
				jQuery("#fileName")
				.fadeIn('slow')
				.append('<div id="sortable_'+nums+'"><div id="gallery_uploaded"><img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/thumbnails/'+eiptc[0].replace("%","%25")+'" align="absmiddle" style="height:90px;width:130px;"></div></div>');	
			<?php } ?>
		} else {
            <?php if(!function_exists('gd_info')) { ?>
				jQuery("#fileName")
				.fadeIn('slow')
				.append('<div id="sortable_'+nums+'"><div id="gallery_uploaded"><img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/'+eiptc[0].replace("%","%25")+'" align="absmiddle"></div></div>');
			<?php } else { ?>
				jQuery("#fileName")
				.fadeIn('slow')
				.append('<div id="sortable_'+nums+'"><div id="gallery_uploaded"><img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/thumbnails/'+eiptc[0].replace("%","%25")+'" align="absmiddle"></div></div>');
			<?php } ?>
	    }
        ...
        if (answer){
            ...
            jQuery('[id^=delete_img_]:checked').each(function(index) {
				...
				setTimeout(function(){
					...
					jQuery.ajax({type: "POST", url: "<?php echo get_bloginfo('template_url'); ?>/admin/gallery/delete-gallery-images.php", data: "gallery_id=<?php echo $_GET['post']; ?>&img_id="+currentId.substr(11), success: function(data)
					{
    ...
    jQuery('#meta_button').click(function(){
    ...
    jQuery('#meta_button_close').click(function(){
    ...
    jQuery('#ecommerce_button').click(function(){
        jQuery('.ui-state-default').css('clear','both');
        jQuery('.ui-state-default').css('width','100%');
    ...
    jQuery('#ecommerce_button_close').click(function(){
        jQuery('.ui-state-default').css('clear','none');
        jQuery('.ui-state-default').css('width','auto');
        jQuery('[id^=gallery_options_]').hide();
        jQuery('[id^=gallery_uploaded_]').css('margin-top','2px');
	...
	jQuery('[id^=ecomm_options_master_]').change(function(){
		...
		if(jQuery(this).val() == 'ON') {
			...
			jQuery('[id^=ecomm_options_'+currentId.substr(21)+'_]').each(function() {
				
				jQuery(this).attr('checked','checked');
				
			});
			
		} else if(jQuery(this).val() == 'OFF') {
    ...
    jQuery('#import_nexgen').click(function(){
		...
		jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/get-nextgen-image.php", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
		{
    ...
    jQuery('#import_nexgen_images').live('click', function(){
        ...
        if (answer){
            ...
            jQuery('[id^=nggallery_image_]').each(function(index) {
                ...
                jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/import-nextgen-images.php", data: 'image='+fileName.replace("&","%26")+'&path='+jQuery('#nggallery_path_'+currentId.substr(16)).val()+'&gallery_id=<?php echo $_GET['post']; ?>', success: function(data)
				{  
                    ...
                    jQuery("#fileName")
                    .fadeIn('slow')
                    .append('<div id="sortable_'+nextsize+'"><div id="gallery_uploaded"><img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/thumbnails/'+fileName.replace("%","%25").replace("&","&amp;")+'" align="absmiddle">')
                    .append('</div></div>');
    ...
    jQuery('#import_photocrati').click(function(){
		...
		jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/get-photocrati-image.php", data: 'post=<?php echo $_GET['post']; ?>', success: function(data)
		{
    ...
    jQuery('#import_photocrati_images').live('click', function(){
        ...
        if (answer){
            ...
            jQuery('[id^=photocrati_image_]').each(function(index) {
                ...
                var fileTitle = jQuery(this).attr('title');
                var fileDesc = jQuery(this).attr('alt');
                jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/import-photocrati-images.php", data: 'image='+fileName.replace("&","%26")+'&path='+jQuery('#photocrati_path_'+currentId.substr(17)).val()+'&gallery_id=<?php echo $_GET['post']; ?>', success: function(data)
				{  
	...
	jQuery('[id^=save_button_]').live('click', function(){
		...
		function saveGallery() {
			...
			jQuery('[id^=sortable_]').each(function(index) {
				...
				jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-gallery-images-update.php", data: 'gallery_id=<?php echo $_GET['gallery_id']; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&image_name='+jQuery('#gallery_image_'+currentId.substr(9)).val().replace("&","%26")+'&image_order='+jQuery('#image_order_'+currentId.substr(9)).val()+'&image_alt='+jQuery('#image_alt_'+currentId.substr(9)).val().replace("&","%26")+'&image_desc='+jQuery('#image_desc_'+currentId.substr(9)).val().replace("&","%26")+'&ecomm_options='+options, success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/update-gallery.php", data: 'gallery_id=<?php echo $_GET['gallery_id']; ?>&post_id=<?php echo $_GET['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26")+'&gal_desc='+jQuery("#gal_desc").val().replace("&","%26")+'&gal_height='+jQuery("#gal_height").val(), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
						
								var img_html = '<img id="phgallery-<?php echo $_GET['gallery_id']; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php echo get_bloginfo('template_url'); ?>/admin/images/gallery-placeholder-'+jQuery("#gal_number").val()+'.gif" alt="photocrati gallery" />';
							...
							if (jQuery.browser.msie && jQuery.browser.version.slice(0,1) == '9')
							{
								ajax_async = false;
							}
							
							jQuery.ajax({type: "POST", async: ajax_async, url: "<?php echo get_bloginfo('template_url'); ?>/admin/gallery/get_galleries.php", data: 'post_id=<?php echo $_GET['post']; ?>', success: function(data)
								{
...
<form name="create_gallery" id="create_gallery" method="post">
    ...
    <div id="gallery_types">
        ...
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_6" value="6" <?php if($galtype == 6) { echo 'checked="checked"'; } ?> /> Album - List<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/album_list.jpg" id="gal_img_6" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_7" value="7" <?php if($galtype == 7) { echo 'checked="checked"'; } ?> /> Album - Grid<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/album_grid.jpg" id="gal_img_7" /></div>
    ...
    <h2>Gallery Type:</h2>
    	...
    	<div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_1" value="1" <?php if($galtype == 1) { echo 'checked="checked"'; } ?> /> Slideshow<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_animated.jpg" id="gal_img_1" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_2" value="2" <?php if($galtype == 2) { echo 'checked="checked"'; } ?> /> Blog Style<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_sstack.jpg" id="gal_img_2" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_3" value="3" <?php if($galtype == 3) { echo 'checked="checked"'; } ?> /> Filmstrip<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_hfilm.jpg" id="gal_img_3" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_4" value="4" <?php if($galtype == 4) { echo 'checked="checked"'; } ?> /> Thumbnails<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_thumbs.jpg" id="gal_img_4" /></div>
        <div class="types_inner"><input type="radio" name="gal_type" class="gal_type" id="gal_type_5" value="5" <?php if($galtype == 5) { echo 'checked="checked"'; } ?> /> E-Commerce<BR /><img src="<?php echo get_bloginfo('template_directory'); ?>/admin/images/gallery_ecomm.jpg" id="gal_img_5" /></div>
    ...
    <div id="gallery_title">
		...
		<b>IMPORTANT NOTES:</b> This only applies to Slideshow and Filmstrip galleries. You can use this setting to set the main image area height
		if you are uploading wide panoramic images.
    ...
    <p>A maximum file size of 2MB per photo applies. <b>There is a maximum upload limit of 99 images at one time</b>.</p>
    ...
    <div style="float:left;width:18%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/gallery_nexgen.jpg" id="import_nexgen" value="Import from NexGen" onclick="return false;" />
    </div>
    <div style="float:left;width:18%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/gallery_photocrati.jpg" id="import_photocrati" value="Import from Photocrati" onclick="return false;" />
    </div>
    <div style="float:left;width:17%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/save_gallery.jpg" id="save_button_1" value="Save Gallery" onclick="return false;" />
    </div>
    <div id="msggallery1"></div>
	<div id="msggallery3"></div>
    <div style="float:left;width:18%;clear:both;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/meta_gallery.jpg" id="meta_button" value="Edit Meta Data" onclick="return false;" />
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/meta_gallery_close.jpg" id="meta_button_close" value="Close Meta Data" onclick="return false;" style="display:none;" />
	</div>
    <div style="float:left;width:18%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/ecommerce_gallery.jpg" id="ecommerce_button" value="Edit Ecommerce Data" onclick="return false;" />
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/ecommerce_gallery_close.jpg" id="ecommerce_button_close" value="Close Ecommerce Data" onclick="return false;" style="display:none;" />
	</div>
    <div style="float:left;width:64%;">
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/delete_selected.jpg" id="delete_selected" value="Delete Selected" onclick="return false;" />
	</div>
	...
	<div id="ecommerce_note" style="display:none;margin-bottom:10px;">
		...
		<p><label><strong>Include / Exclude Options Gallery Wide:</strong></label></p>
					
		<?php
		$ecommsettings = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."photocrati_ecommerce_settings WHERE id = 1", ARRAY_A);
		foreach ($ecommsettings as $key => $value) {
			$$key = $value;
		}
		$l = 1;
		while($l < 21) {
		?>
			...
			<div class="ecomm_op" style="float:left;width:33%;<?php if(!${"ecomm_op$l"}) { echo 'display:none;'; } ?>">
				<select id="ecomm_options_master_<?php echo $l; ?>">
					<option value=""> </option>
					<option value="ON">ON</option>
					<option value="OFF">OFF</option>
				</select> <?php echo ${"ecomm_op$l"}; ?>
			</div>
	...
	?>
        ...
        <div id="galleria_<?php echo $gallery->id; ?>">
            <div class="gallery_uploaded" id="gallery_uploaded_<?php echo $gallery->id; ?>">
                <div class="image">
					<?php if(!function_exists('gd_info')) { ?>
						<?php if (file_exists($upload_dir['basedir'].'/galleries/post-'.$_GET['post'].'/'.str_replace("&amp;","&",$gallery->image_name))) { ?>
							<img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/<?php echo str_replace("%","%25",$gallery->image_name); ?>" align="absmiddle" style="max-height:100px;max-width:130px;">
						<?php } else { ?>
							<img src="<?php echo bloginfo('template_url'); ?>/galleries/post-<?php echo $_GET['post']; ?>/<?php echo str_replace("%","%25",$gallery->image_name); ?>" align="absmiddle" style="max-height:100px;max-width:130px;">
						<?php } ?>
					<?php } else { ?>
						<?php if (file_exists($upload_dir['basedir'].'/galleries/post-'.$_GET['post'].'/'.str_replace("&amp;","&",$gallery->image_name))) { ?>
							<img src="<?php echo $upload_dir['baseurl']; ?>/galleries/post-<?php echo $_GET['post']; ?>/thumbnails/<?php echo str_replace("%","%25",$gallery->image_name); ?>" align="absmiddle" style="max-height:100px;max-width:130px;">
						<?php } else { ?>
							<img src="<?php echo bloginfo('template_url'); ?>/galleries/post-<?php echo $_GET['post']; ?>/thumbnails/<?php echo str_replace("%","%25",$gallery->image_name); ?>" align="absmiddle" style="max-height:100px;max-width:130px;">
						<?php } ?>
					<?php } ?>
            ...
            <p><label>Title / Alt:</label><BR><input type="text" size="45" id="image_alt_<?php echo $nums; ?>" value="<?php if($gallery->image_alt) { echo htmlspecialchars_decode(stripslashes($gallery->image_alt)); } else { echo htmlspecialchars_decode($gallery->image_name); } ?>"></p>
            <p><label>Caption:</label><BR><textarea cols="50" rows="2" id="image_desc_<?php echo $nums; ?>"><?php echo htmlspecialchars_decode(stripslashes($gallery->image_desc)); ?></textarea></p>
            </div>
            <div class="gallery_meta" id="gallery_options_<?php echo $gallery->id; ?>" style="display:none;overflow:visible;height:auto;">
				<p><label>Include / Exclude Options:</label></p>
					...
					<?php
					$ecomm_ops = explode(',',$gallery->ecomm_options);
					$l = 1;
					while($l < 21) {
					?>
					
						<div class="ecomm_op"<?php if(!${"ecomm_op$l"}) { echo ' style="display:none;"'; } ?>>	
							<input name="ecomm_options_<?php echo $nums; ?>" id="ecomm_options_<?php echo $l; ?>_<?php echo $gallery->id; ?>" type="checkbox" <?php if(in_array($l,$ecomm_ops) || !${"ecomm_op$l"}) { echo 'checked="checked"'; } ?> value="<?php echo $l; ?>"> <?php echo ${"ecomm_op$l"}; ?>
						</div>
    ...
    <input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/save_gallery.jpg" id="save_button_2" value="Save Gallery" onclick="return false;" />
    </div>
