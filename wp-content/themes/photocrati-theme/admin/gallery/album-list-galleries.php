...
jQuery(document).ready(function()
{
	...
	jQuery('[id^=save_album_]').click(function(){
	
		function saveAlbum() {
			var loop = 0;
			var galsize = jQuery("input[name='album']:checked").size();
			jQuery("input[name='album']:checked").each(function(index) {
				jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-album-galleries.php", data: 'album_id=<?php echo $_POST['post']; ?>_<?php echo $galnextid; ?>&gallery_id='+jQuery(this).val()+'&album_order='+jQuery('#album_order_'+jQuery(this).val()).val(), success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-gallery.php", data: 'gallery_id=<?php echo $_POST['post']; ?>_<?php echo $galnextid; ?>&post_id=<?php echo $_POST['post']; ?>&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26")+'&gal_type='+jQuery("input[name='gal_type']:checked").val(), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
                            ...
                            if (parent.tinyMCE.activeEditor != null && parent.tinyMCE.activeEditor.isHidden() == false) {
                                ...
                                parent.tinyMCE.execCommand('mceInsertContent',false,'<img id="phgallery-<?php echo $_POST['post']; ?>_<?php echo $galnextid; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php bloginfo('template_url'); ?>/admin/images/album-placeholder-<?php echo $galnextid; ?>.gif" alt="photocrati gallery" />');
								parent.jQuery('[id^=the_content_]').val(parent.document.post.content.value);
							
                            } else {
                                
                                insertInPost('<img id="phgallery-<?php echo $_POST['post']; ?>_<?php echo $galnextid; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php bloginfo('template_url'); ?>/admin/images/album-placeholder-<?php echo $galnextid; ?>.gif" alt="photocrati gallery" />');
								parent.jQuery('[id^=the_content_]').val(parent.document.post.content.value);
							
                            }
							
							jQuery.ajax({type: "POST", url: "<?php echo get_bloginfo('template_url'); ?>/admin/gallery/get_galleries.php", data: 'post_id=<?php echo $_POST['post']; ?>', success: function(data)
								{
	...
	jQuery('[id^=update_album_]').click(function(){
		
	jQuery.ajax({type: "POST", url: "<?php echo get_bloginfo('template_url'); ?>/admin/gallery/delete-album.php", data: 'gallery_id=<?php echo $_POST['gallery_id']; ?>', success: function(data)
	{
		...
		function updateAlbum() {
			...
			jQuery("input[name='album']:checked").each(function(index) {
				...
				jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-album-galleries.php", data: 'album_id=<?php echo $_POST['gallery_id']; ?>&gallery_id='+jQuery(this).val()+'&album_order='+jQuery('#album_order_'+jQuery(this).val()).val(), success: function(data)
					{
						if(loop == 0) {
						jQuery.ajax({type: "POST", url: "<?php bloginfo('template_url'); ?>/admin/gallery/save-gallery.php", data: 'gallery_id=<?php echo $_POST['gallery_id']; ?>&post_id=<?php echo $_POST['post']; ?>&gal_type='+jQuery("input[name='gal_type']:checked").val()+'&gal_title='+jQuery("input[name='gal_title']").val().replace("&","%26"), success: function(data)
						{}});
						}
						loop++;
						if(loop == galsize) {
                            ...
                            if (parent.tinyMCE.activeEditor != null && parent.tinyMCE.activeEditor.isHidden() == false) {
                                ...
                                parent.tinyMCE.execCommand('mceInsertContent',false,'<img id="phgallery-<?php echo $_POST['gallery_id']; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php echo get_bloginfo('template_url'); ?>/admin/images/album-placeholder-'+jQuery("#gal_number").val()+'.gif" alt="photocrati gallery" />');
                            
                            } else {
                                
                                insertInPost('<img id="phgallery-<?php echo $_POST['gallery_id']; ?> '+jQuery("input[name='gal_type']:checked").val()+'" src="<?php echo get_bloginfo('template_url'); ?>/admin/images/album-placeholder-'+jQuery("#gal_number").val()+'.gif" alt="photocrati gallery" />');
...
<?php if($_POST['edit']) { ?>
<input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/update_album.jpg" id="update_album_1" value="Update Album" onclick="return false;" />
<?php } else { ?>
<input type="image" src="<?php bloginfo('template_url'); ?>/admin/images/save_album.jpg" id="save_album_1" value="Save Album" onclick="return false;" />
<?php } ?>
</div>
